...
void oled_p8x16str_spi(uint8 x,uint8 y,const int8 ch[])
{
	...
	while (ch[j]!='\0')
	{
	  	...
	  	oled_set_pos_spi(x,y);
	  	for(i=0;i<8;i++)	oled_wrdat_spi(oled_8x16[c*16+i]);
	  	  
	  	oled_set_pos_spi(x,y+1);
	  	for(i=0;i<8;i++)	oled_wrdat_spi(oled_8x16[c*16+i+8]);
...
void oled_printf_int32_spi(uint16 x,uint16 y,int32 dat,uint8 num)
{
    ...
    oled_p6x8str_spi(x, y, buff);	//显示数字
}
